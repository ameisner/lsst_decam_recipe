...
This exposure is centered at (RA, DEC) ~ (354, -3), so the reference catalogs can be taken care of by simply making sure that the usual `DATA/ref_cats` directory contains the appropriate list of standard PS1 shard files. Standard Butler repo setup suffices and a standard call to `processCcd.py` can correspondingly be used:
